...
--- kuidas muuta sp-d võtit peale et leegi teine peale teie
--- ei saaks seda muuta
...
---annab tulmuse kus loendab ära nõuetele vastavd read
...
-- tabeli info
sp_help Employees
-- kui soovd sp teksti näha
úp_helptext spGetEmployeesCountByGender

-- vaatame millest sõltub see sp
sp_depends spGetEmployeescountByGender
--- saame teada mitu asja sõltub sellest tabelsit 
...
--- see konverseerib ASCII tähe väärtuse numbriks
select ASCII('a')
--- näitab A-tähte
select CHAR (69)
...
--- vastavalt upper ja lower-ga saan muutqa märkide suurust
--- reverse funktsioon pöörab kõik ümber
select REVERSE(UPPER(ltrim(FirstName))) as FirstName, MiddleName, LOWER(LastName),
RTRIM(LTRIM(FirstName)) + ' ' + MiddleName as FullName
from Employees

--näitab plaju tähe märke mitu on nimes ja loeb tühikud sisse
select FirstName, LEN(FirstName) as [Total Characters] from Employees

--- näeb ära mitu tähte on sõnal ja ei loe tühikud sisse
...
---esimene nr peale komakohta näitab mitmendas alustab
...
----peale @ tähemärki reguleerin tähhe pikkuse näitamist
...
select FirstName, LastName,
	SUBSTRING(Email,1, 2) + REPLICATE('*', 5) + --- peale teist tähemärki paneb viis
	SUBSTRING(Email, CHARINDEX('@', Email), len(Email) - CHARINDEX('@', Email)+1) as Email --- kuni @- märgini on dünaamiline0
from Employees

--- kolm korda näitab stringis olevat väärtust
select REPLICATE('asd', 3)

--- kuidas sisestada tühikud kahe nime vahele
select SPACE(5)
...
--- PATINDEX
--- sama mis CHARINDEX aga dünaamiliselm ja saab kasutada wildcardi
